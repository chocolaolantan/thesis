...
public class W2vModel {
  ...
  public float[] getWordVector(int n) {
    if (n > words || n < 0) return null;
    if (exm) return m[n];
    else return getWv(n);
  }
  public float[][] getVectors(int[] list) {
    if (list == null || list.length <= 0) return null;
    float[][] res = new float[list.length][];
    for (int i = 0; i < list.length; i++) {
      if (list[i] < 0 || list[i] > words) {
  ...
  public float[][] getAllVector() { return m; }

  public int[] getNearWords(int idx, int n, int[] excl) {
    if (n > words || n < 0) return null;
    if (exm) return gNWm(idx, n, excl);
    else return gNWnm(idx, n, excl);
  }
  public int[] getVectorNearWords(float[] grv, int n, int[] excl) {
    if (n > words || n < 0) return null;
